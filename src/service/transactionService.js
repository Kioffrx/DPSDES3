...
export const agregarTransaccion = async (monto, tipo, categoria, cuenta, fecha, descripcion) => {
  try {
    const usuarioLogueado = auth.currentUser;
    if (!usuarioLogueado) throw new Error("No hay sesión activa");

    // Guardamos en la colección 'transacciones' ligada al UID del usuario [doc: 51]
    const docRef = await addDoc(collection(db, 'transacciones'), {
      uidUsuario: usuarioLogueado.uid, // Estricto para la rúbrica [doc: 51]
      monto: parseFloat(monto),
      tipo,         // 'ingreso' o 'gasto'
      categoria,    // ej. 'Comida', 'Transporte'
      cuenta,       // ej. 'Efectivo', 'Tarjeta'
...
export const obtenerTransacciones = async (filtros = {}) => {
  try {
    ...
    let consulta = query(
      ...
      where('uidUsuario', '==', usuarioLogueado.uid)
    );

    // Filtros dinámicos (Requisito de Persona 1) [doc: 16]
    if (filtros.categoria) {
      consulta = query(consulta, where('categoria', '==', filtros.categoria));
    }
    if (filtros.cuenta) {
      consulta = query(consulta, where('cuenta', '==', filtros.cuenta));
    }
    ...
    querySnapshot.forEach((documento) => {
      transacciones.push({ id: documento.id, ...documento.data() });
    });
...
export const editarTransaccion = async (idTransaccion, datosActualizados) => {
  try {
    const docRef = doc(db, 'transacciones', idTransaccion);
    await updateDoc(docRef, {
      ...datosActualizados,
      monto: parseFloat(datosActualizados.monto)
    });
  } catch (error) {
    console.error("Error al editar transaccion:", error);
    throw error;
  }
};
